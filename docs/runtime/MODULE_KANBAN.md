...
- `X00-T20` Long-session media slowdown — in-progress; bounded clock-rate compensation passed the Debug build, 15 timing/adaptation cases and media-transport CTest, then was deployed with rollback retained. Post-deployment feedback froze after capture generation changes; pacing fell from 3806 to 638 kbps and transmission from about 22.6 to 4.25 FPS. Authorized follow-up fixes in-epoch capture sequence resets and verifies NVENC forced IDR configuration. Three focused recovery cases, one developer-selected local NVENC H.264 standalone-IDR case and one Debug NoPublish main build passed; earlier timing/transport evidence is reused. Next checkpoint: authorized push and full-directory Host upgrade with rollback. Individual 3–4-second stalls, long-session field effectiveness and complete incident resolution remain unverified. See [diagnostic handoff](../testing/desktop-latency-quality-investigation-20260915.md).
