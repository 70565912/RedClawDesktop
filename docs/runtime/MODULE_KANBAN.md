...
- `X00-T20` Long-session media slowdown — in-progress; feedback-driven adaptive sender `9324053` implemented, locally verified, pushed and deployed to Host with explicit authorization. Nine new automatic cases and affected regressions passed; prior Debug candidate and tests were reused for deployment. Independent full-directory upgrade and Host restart completed with both installed 337-file and rollback manifests verified; operational settings preserved except coordination revision. Peer subsequently reconnected: streaming=true, capture/encode/transmit nonzero, synthetic=0. The earlier Controller keyframe-gap diagnosis remains recorded in the development log. Field FPS/latency improvement and long-session effectiveness remain unverified. Next: bounded matched-scene metrics, without another full matrix. See [diagnostic handoff](../testing/desktop-latency-quality-investigation-20260915.md).
