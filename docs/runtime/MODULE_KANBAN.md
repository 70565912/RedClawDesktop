...
- `X00-T20` Long-session media slowdown — in-progress; the first feedback-driven adaptive sender was deployed, then a 39-second field window confirmed a latency regression: 10.3–16.8 FPS periodic transmit, 14-frame pending peak, 426 ms oldest pending age and a learned target near 942 ms. The follow-up candidate removes receiver-rate self-lock, keeps ordinary media moving while a probe awaits feedback, uses cumulative bounded-catch-up frame timing and prevents keyframe history from expanding the ordinary FIFO. One affected automatic target and the Debug main candidate pass. Next: deploy with rollback, reconnect and compare bounded live FPS/queue/stall metrics against both this regression and the original 19.384 FPS / 339–574 ms keyframe-gap baseline. See [diagnostic handoff](../testing/desktop-latency-quality-investigation-20260915.md).
