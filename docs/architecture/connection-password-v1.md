...
本机密码首次设置和修改均需输入两次。等待或连接期间不可修改；先退出当前会话。密码非空，保留首尾空格、区分大小写，无复杂度要求。协议上限为 1024 UTF-8 字节，不接受 NUL。界面默认隐藏密码，派生校验材料在后台完成。
...
- `redclaw_ui_connection_flow_tests` 中 `ConnectionEntryPage.*`：输入确认、异步保存、当前用户密文、成功后记忆、忘记、解密失败和运行期禁改。
