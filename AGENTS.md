...
替换正在提供会话的 Host 时，先完成新程序构建，并准备好不依赖该会话、失败时能拉起替换前程序的恢复任务。确认对端已经重新发布会话后，才结束旧进程。远程终端和工作台通道随该 Host 一起消失；只重启 Controller 不会让 Host 上线。`connected=false` 且 `remote_revision=0`、`fetch_hits=0` 表示 Host 没有发布会话。详见技能中的「替换正在提供会话的 Host」。
