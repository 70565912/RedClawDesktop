param(
    ...
    [string]$DestinationRoot = "build/reports/m07-t02-runs",
    [string]$ManifestFileName = "evidence-manifest.json",
    [string]$BaselineFileName = "m07-t02-boot-validation.json",
    [string]$PostRebootFileName = "m07-t02-post-reboot-evidence.json",
    [switch]$Strict
)
...
$resolvedSourceRoot = Resolve-RepoPath -PathValue $SourceRoot -RepoRoot $repoRoot
$resolvedDestinationRoot = Resolve-RepoPath -PathValue $DestinationRoot -RepoRoot $repoRoot
$destinationRunDir = Join-Path $resolvedDestinationRoot $RunId

$expectedFiles = @($BaselineFileName, $PostRebootFileName)
