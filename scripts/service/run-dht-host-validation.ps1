...
$reportDirectory = Join-Path $resolvedReportRoot ("dht-host-validation-{0}" -f $timestamp)
$stdoutPath = Join-Path $reportDirectory "host.out.log"
$stderrPath = Join-Path $reportDirectory "host.err.log"
...
Write-Host "[dht-host] report: $reportDirectory"
Write-Host "[dht-host] runtime: $resolvedRuntimeExe"
Write-Host "[dht-host] session: $(Get-RedactedSessionCode -Code $SessionCode)"
...
if ($bootstrapReadiness.blocked -and -not $DryRun) {
    ...
    Write-Host "[dht-host] bootstrap readiness blocked runtime start: $($bootstrapReadiness.status)"
    Write-Host "[dht-host] summary: $summaryPath"
    exit 1
}

if ($DryRun) {
    Write-Host "[dht-host] dry run only; runtime was not started."
    Write-Host "[dht-host] redacted args: $($redactedArguments -join ' ')"
    exit 0
}
...
try {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ($null -eq $connectedAt -and (Test-ConnectedFromLog -Text $stdoutText)) {
            ...
            Write-Host "[dht-host] connected signal observed; holding for $StopAfterConnectedSeconds second(s)."
        }
    ...
    if (-not $hostProc.HasExited -and -not $stoppedAfterSuccess) {
        $timedOut = $true
        Stop-Process -Id $hostProc.Id -Force
    }
} finally {
    if ($null -ne $hostProc -and -not $hostProc.HasExited) {
        Stop-Process -Id $hostProc.Id -Force
    }
}
...
$summary = @{
    ...
    dht_signal_published = ($finalStdout -match "Runtime DHT signal published role=host")
    remote_description_applied = ($finalStdout -match "Runtime remote description applied role=host")
    last_runtime_state = Get-LastMatchingLine -Text $finalStdout -Pattern "Runtime state role=host"
    last_candidate_stats = Get-LastMatchingLine -Text $finalStdout -Pattern "Runtime candidate stats role=host"
    last_dht_stats = Get-LastMatchingLine -Text $finalStdout -Pattern "Runtime DHT stats role=host"
    last_dht_backend_diagnostics = Get-LastMatchingLine -Text $finalStdout -Pattern "Runtime DHT backend diagnostics role=host"
...
Write-Host "[dht-host] status: $status"
Write-Host "[dht-host] summary: $summaryPath"
if ($summary.last_runtime_state) {
    Write-Host "[dht-host] $($summary.last_runtime_state)"
}
if ($summary.last_candidate_stats) {
    Write-Host "[dht-host] $($summary.last_candidate_stats)"
}
if ($summary.last_dht_stats) {
    Write-Host "[dht-host] $($summary.last_dht_stats)"
}
