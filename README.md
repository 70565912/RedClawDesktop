...
| 远程系统声音 | 可用；Controller 开启后 Host 采集共享模式系统声音。播放期间输出保持打开，关闭远程声音或结束会话才释放。两端都需具备音频能力 |
...
6. 画面出现后，按界面提示显式开启远程输入。需要听远端声音时，在 Controller 打开远程声音；对端仍是 v0.1.4 时该按钮不可用。Agent 功能需要 Host 端配置允许的项目和 Provider。
